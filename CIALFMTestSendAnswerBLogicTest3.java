...
public class CIALFMTestSendAnswerBLogicTest3 extends BaseJunitTestVariable{
    ...
    public void fileMake(String fileName) throws IOException {
        file = new File(String.format("junitTestLog/外貨資格更新テスト回答送信BLogic動作確認3/%s.txt",fileName));
    ...
    public void playTest(int testNo,String[] periodList,int fMAdDayCount,String lastFMAdDate, String lastFMPassDate,int lastFMScore, int lastFMPassNo,int okCnt,boolean passFlg) {
        try {
            fileMake(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%d",testNo));
        } catch (IOException e) {
            e.printStackTrace();
        }
        pw.println(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%dデータ準備開始",testNo));
        ...
        pw.println(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%dデータ準備終了",testNo));
        pw.println(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%d開始",testNo));
        pw.println("●結果");
        pw.println(testSendAnswerBLogic.execute(paramP));
        userData.selectFMUserInfo();
        foreignHistoryData.selectForeignHistoryInfo();
        pw.println(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%d終了",testNo));
        pw.println(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%dデータ戻し開始",testNo));
        periodData.updateFMPeriodDefault();
        userData.updateUserFMInfoClear();
        pw.println(String.format("外貨資格更新テスト回答送信BLogic動作確認3_No%dデータ戻し終了",testNo));
        pw.close();
    }
    @Test
    public void test外貨資格更新テスト回答送信BLogic動作確認3_No1(){playTest(1,new String[] {today,todayPlus30,todayPlus31,todayPlus61},0,null,null,0,0,5,false);}
    @Test
    public void test外貨資格更新テスト回答送信BLogic動作確認3_No2(){playTest(2,new String[] {today,todayPlus30,todayPlus31,todayPlus61},0,null,null,0,0,5,false);}
    @Test
    public void test外貨資格更新テスト回答送信BLogic動作確認3_No3(){playTest(3,new String[] {today,todayPlus30,todayPlus31,todayPlus61},0,null,null,0,0,5,true);}
    @Test
    public void test外貨資格更新テスト回答送信BLogic動作確認3_No4(){playTest(4,new String[] {today,todayPlus30,todayPlus31,todayPlus61},0,null,null,0,0,5,true);}
}
